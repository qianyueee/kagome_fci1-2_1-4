"""Nb=5 trap×2 (V_trap=0.010), two points on V_nn=V_nnn diagonal:
(V_nn, V_nnn) = (5, 5) and (6, 6).  Per point: lowest-100 spectrum +
C6-symm GS density (orbit-averaged).
"""
...
lat = DiskLattice()
params = ModelParams(V_trap=0.010)   # 2x default
Nb = 5
k_per_sector = 30
...
for V_nn, V_nnn in points:
    ...
    print(f"  Lowest 5: " + ", ".join(
        f"E={E100[k]:.4f}(L={L100[k]})" for k in range(5)))
    ...
    tag = f'vnn{V_nn:g}_vnnn{V_nnn:g}_trap2x'.replace('.', 'p')
    ...
    ax1.set_title(rf"$V_{{trap}}={params.V_trap:g}$ (2$\times$ default)",
                  fontsize=12)
    ...
    print(f"  Sublattice:  A,B,C means = "
          f"{density[lat.sublats==0].mean():.4f}, "
          f"{density[lat.sublats==1].mean():.4f}, "
          f"{density[lat.sublats==2].mean():.4f}")

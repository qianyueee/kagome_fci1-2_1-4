"""Nb=5 V_nnn = 1.5 V_nn scan, 72-site Kagome disk, discrete paper-trap.

Paper trap (He thesis / [53]):  V = V_trap Σ |r|^2 n_r,  V_trap = 0.005
with |r| in units of a/2.  Continuous form: V ≈ 0.02 d^2 (d in units of a).

Discretized using the newtrap stepping:
    E_trap(r) = round((x^2 + y^2) * 9) * (0.02 / 9)
              ~ 0.02 * d^2   (continuous limit)

V_nn from 4.0 to 6.0, 11 points (step 0.2).  Lowest 10 states per (L sector, V).
...
TRAP_COEF = 0.02 / 9.0  # discrete form of paper trap V_trap |r|^2 (|r| in a/2 units)


def build_static_steptrap005(lattice, params, V_nn=0.0, V_nnn=0.0):
    n = lattice.n_sites
    tol = 1e-6
    ...
    for i in range(n):
        ...
        e = round((pos[0] ** 2 + pos[1] ** 2) * 9) * TRAP_COEF
...
def run_ed_steptrap005(lattice, params, Nb, V_nn=0.0, V_nnn=0.0, n_states=30):
    static = build_static_steptrap005(lattice, params, V_nn=V_nn, V_nnn=V_nnn)
    sigma = lattice.sigma
...
V_nn_list = np.linspace(4.0, 6.0, 11)
V_nnn_list = 1.5 * V_nn_list
tag = 'papertrap_disc_vnnn_1p5vnn_4to6'
...
for idx, (v, vp) in enumerate(zip(V_nn_list, V_nnn_list)):
    ...
    print(f"\n=== [{idx+1}/{len(V_nn_list)}] V_nn={v:.4f}  V_nnn={vp:.4f} ===",
          flush=True)
    E, L = run_ed_steptrap005(lat, params, Nb=Nb, V_nn=v, V_nnn=vp,
                              n_states=n_show)
    k = min(n_show, len(E))
    records_E[idx, :k] = E[:k]
    records_L[idx, :k] = L[:k]
    ...
    np.savez(npz_out, V_nn=V_nn_list, V_nnn=V_nnn_list,
             ...
             trap_formula=f'round((x^2+y^2)*9)*{TRAP_COEF}')
...
for Ltag in range(6):
    ...
    ax.scatter(xs, ys, s=22, alpha=0.85, color=cmap(Ltag),
               label=f'L={Ltag}', edgecolor='none')

ax.set_xlabel(r'$V_{nn}$  ($V_{nnn}=1.5\,V_{nn}$, '
              r'discrete paper-trap $V_{trap}=0.005$, $|r|$ in $a/2$)')
ax.set_ylabel('E')
ax.set_title(f'Nb={Nb} spectrum vs V '
             f'(disc. $V_{{trap}}|r|^2$ with $V_{{trap}}=0.005$, '
             f'$V_{{nnn}}=1.5V_{{nn}}$, '

"""Nb=5 V_nnn = V_nn / 1.5 scan: V_nn from 2 to 6, 21 points (step 0.2),
keeping only the lowest 10 states per (L sector, V) point.
"""
...
V_nn_list = np.linspace(2.0, 6.0, 21)
V_nnn_list = V_nn_list / 1.5
tag = 'vnnn_1p5_vnn_2to6'
...
ax.set_xlabel(r'$V_{nn}$  ($V_{nnn}=V_{nn}/1.5$)')
ax.set_ylabel('E')
ax.set_title(f'Nb={Nb} spectrum vs V  '
             f'($V_{{nnn}}=V_{{nn}}/1.5$, {len(V_nn_list)} pts, lowest {n_show})')

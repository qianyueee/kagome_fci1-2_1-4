"""Nb=5 Coulomb-like scan with V_nnn = V_nn / sqrt(3): V_nn from 0 to 4, 21 points.

The ratio sqrt(3) ≈ 1.732 reflects the geometric distance ratio between
NNN (a*sqrt(3)/2) and NN (a/2) on the Kagome lattice. So V_nn : V_nnn
matches the 1/r Coulomb tail truncated at NNN.
"""
...
V_nn_list = np.linspace(0.0, 4.0, 21)
V_nnn_list = V_nn_list / np.sqrt(3.0)
tag = 'coulomb_vnnn_sqrt3_vnn_0to4'
...
ax.set_xlabel(r'$V_{nn}$  ($V_{nnn}=V_{nn}/\sqrt{3}$, 1/r ratio)')
ax.set_ylabel('E')
ax.set_title(f'Nb={Nb} spectrum vs Coulomb-like V  '
             f'(0 to 4, {len(V_nn_list)} pts, lowest {n_show})')

"""Nb=5 V_nn=4 V_nnn=4 with trap doubled (V_trap=0.010 instead of 0.005):
lowest-100 spectrum + C6-symm GS density.  Same procedure as
ed_density_nb5_vnn4_vnnn4.py but with overridden ModelParams(V_trap=0.01).
"""
...
lat = DiskLattice()
params = ModelParams(V_trap=0.010)   # 2x the default 0.005
Nb = 5
V_nn = 4.0
V_nnn = 4.0
k_per_sector = 30
...
print(f"Nb={Nb}, V_nn={V_nn}, V_nnn={V_nnn}, V_trap={params.V_trap} (2x default)")
...
for k in range(n_keep):
    print(f"  k={k:3d}  E={E100[k]:+.6f}  dE={E100[k]-E100[0]:+.6f}  L={L100[k]}")
...
print(f"  done in {time.time()-ts:.1f}s, sum={density.sum():.4f}")

out_npz = 'data/nb5/kagome_density_nb5_vnn4_vnnn4_trap2x.npz'
...
out_dir = 'figures/nb5/vnn4_vnnn4_trap2x'
...
plt.tight_layout()
out_sp = f'{out_dir}/spectrum_vnn4p0_vnnn4p0_trap2x_n{n_keep}.png'
...
plt.tight_layout()
out_d = f'{out_dir}/density_vnn4p0_vnnn4p0_trap2x.png'

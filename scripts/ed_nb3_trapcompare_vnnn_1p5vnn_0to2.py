"""Nb=3 V_nnn = 1.5 V_nn scan on 72-site Kagome disk, V_nn from 0 to 2 (step 0.2).
Runs both trap forms:
  oldtrap:  V_trap * (4|r|)^2     (continuous, V_trap=0.005)
  newtrap:  round((x^2 + y^2)*9) * 0.02   (discrete)

n_show=20 lowest states per (L sector, V) point.
"""
...
from kagome.hamiltonian import PLUS_PAIRS, quspin_static_lists


def build_static(lattice, params, V_nn=0.0, V_nnn=0.0, trap='old'):
    """trap='old' uses ModelParams.V_trap*(4|r|)^2; trap='new' uses
    round((x^2+y^2)*9)*0.02."""
    if trap == 'old':
        return quspin_static_lists(lattice, params, V_nn=V_nn, V_nnn=V_nnn)
...
def run_ed(lattice, params, Nb, V_nn, V_nnn, n_states, trap):
    static = build_static(lattice, params, V_nn=V_nn, V_nnn=V_nnn, trap=trap)
    sigma = lattice.sigma
...
def scan_and_plot(lat, params, Nb, V_nn_list, V_nnn_list, n_show, trap, tag):
    ...
    for idx, (v, vp) in enumerate(zip(V_nn_list, V_nnn_list)):
        if idx < start_idx:
            continue
        print(f"[{trap}][{idx+1}/{len(V_nn_list)}] V_nn={v:.3f}  V_nnn={vp:.3f}",
              flush=True)
        E, L = run_ed(lat, params, Nb=Nb, V_nn=v, V_nnn=vp,
                      n_states=n_show, trap=trap)
        k = min(n_show, len(E))
        records_E[idx, :k] = E[:k]
        records_L[idx, :k] = L[:k]
        ...
        np.savez(npz_out, V_nn=V_nn_list, V_nnn=V_nnn_list,
                 ...
                 trap=trap)
    print(f"[{trap}] total: {time.time()-t0:.1f}s")
    ...
    for Ltag in range(6):
        ...
        for i, v in enumerate(V_nn_list):
            ...
            ys.extend(records_E[i, m].tolist())
        ax.scatter(xs, ys, s=20, alpha=0.85, color=cmap(Ltag),
                   label=f'L={Ltag}', edgecolor='none')
    ax.set_xlabel(r'$V_{nn}$  ($V_{nnn}=1.5\,V_{nn}$, %s trap)' % trap)
    ax.set_ylabel('E')
    ax.set_title(f'Nb={Nb} spectrum vs V ({trap} trap, '
                 f'$V_{{nnn}}=1.5V_{{nn}}$, {len(V_nn_list)} pts, lowest {n_show})')
    ax.legend(loc='upper right', fontsize=9, ncol=2, framealpha=0.9)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    out = f'figures/nb3/kagome_disk_nb{Nb}_{tag}.png'
    plt.savefig(out, dpi=180, bbox_inches='tight')
    plt.close(fig)
    print(f"  saved: {out}\n  saved: {npz_out}")
    return records_E, records_L
...
scan_and_plot(lat, params, Nb, V_nn_list, V_nnn_list, n_show,
              trap='old', tag='oldtrap_vnnn_1p5vnn_0to2')
scan_and_plot(lat, params, Nb, V_nn_list, V_nnn_list, n_show,
              trap='new', tag='newtrap_vnnn_1p5vnn_0to2')

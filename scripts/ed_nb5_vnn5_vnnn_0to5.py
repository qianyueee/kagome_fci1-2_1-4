"""Nb=5 scan at fixed V_nn=5, V_nnn from 0 to 5 (21 points, step 0.25),
lowest 10 states per (L sector, V) point.
"""
...
V_nn_fixed = 5.0
V_nnn_list = np.linspace(0.0, 5.0, 21)
V_nn_list = np.full_like(V_nnn_list, V_nn_fixed)
tag = 'vnn5_vnnn_0to5'
...
if os.path.exists(npz_out):
    ...
    if (prev['V_nnn'].shape == V_nnn_list.shape and
            np.allclose(prev['V_nnn'], V_nnn_list) and
...
for idx, (v, vp) in enumerate(zip(V_nn_list, V_nnn_list)):
    if idx < start_idx:
        continue
    print(f"\n=== [{idx+1}/{len(V_nnn_list)}] V_nn={v:.4f}  V_nnn={vp:.4f} ===",
          flush=True)
...
ax.set_xlabel(r'$V_{nnn}$  ($V_{nn}=5$ fixed)')
ax.set_ylabel('E')
ax.set_title(f'Nb={Nb} spectrum vs $V_{{nnn}}$  '
             f'($V_{{nn}}=5$, {len(V_nnn_list)} pts, lowest {n_show})')

"""Nb=5 V_nn extension scan: V_nn from 6.0 to 10.0, 21 points (step 0.2),
V_nnn=0. Designed to extend the existing 0-6 scan up to V_nn=10.
"""
...
V_nn_list = np.linspace(6.0, 10.0, 21)
npz_out = f'kagome_disk_nb{Nb}_vnn_scan_6to10.npz'
...
ax.set_title(f'Nb={Nb} spectrum vs $V_{{nn}}$  '
             f'(6.0 to 10.0, {len(V_nn_list)} pts, lowest {n_show})')
ax.legend(loc='upper left', fontsize=9, ncol=2, framealpha=0.9)
ax.grid(True, alpha=0.3)

plt.tight_layout()
out = f'kagome_disk_nb{Nb}_vnn_scan_6to10.png'

"""Nb=5 Coulomb-like ext scan: V_nn from 4.2 to 6.0, 10 points (step 0.2),
V_nnn = V_nn / sqrt(3). To be merged with the 0-4 sqrt3 coulomb scan into a 0-6 plot.
"""
...
V_nn_list = np.linspace(4.2, 6.0, 10)
V_nnn_list = V_nn_list / np.sqrt(3.0)
tag = 'coulomb_vnnn_sqrt3_vnn_4p2to6'
...
ax.set_xlabel(r'$V_{nn}$  ($V_{nnn}=V_{nn}/\sqrt{3}$)')
ax.set_ylabel('E')
ax.set_title(f'Nb={Nb} spectrum vs Coulomb-like V  '
             f'(extension 4.2 to 6, {len(V_nn_list)} pts, lowest {n_show})')

"""Plot Nb=3 ground-state site density for three interactions, oldtrap.

Trap: continuous V_trap*(4|r|)^2 with V_trap=0.005.

Cases:
  (i)   V_nn=0,   V_nnn=0     (pure hard-core)
  (ii)  V_nn=1,   V_nnn=1.5
...
cases = [
    ('data/nb3/kagome_disk_nb3_oldtrap_density_freehc.npz',
     r'$V_{nn}=0,\ V_{nnn}=0$ (pure hard-core)'),
    ('data/nb3/kagome_disk_nb3_oldtrap_density_vnn1_vnnn1p5.npz',
     r'$V_{nn}=1,\ V_{nnn}=1.5$'),
    ('data/nb3/kagome_disk_nb3_oldtrap_density_vnn3_vnnn4.5.npz',
...
for k, d in enumerate(data):
    ...
    ax.set_aspect('equal')
    ax.set_title(f"Nb=3 (oldtrap), {d['title']}\n"
...
plt.tight_layout()
out_png = 'figures/nb3/kagome_disk_nb3_oldtrap_density_compare3.png'

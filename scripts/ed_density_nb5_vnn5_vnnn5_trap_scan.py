"""Nb=5 V_nn=V_nnn=5, trap factor 1.0..1.5 step 0.1 (V_trap = factor*0.005).
Per point: lowest-100 spectrum + C6-symm GS density.

Output dir: figures/nb5/vnn5_vnnn5_trap_scan/
"""
...
trap_factors = [1.0, 1.1, 1.2, 1.3, 1.4, 1.5]
V_trap_base = 0.005
...
out_dir = 'figures/nb5/vnn5_vnnn5_trap_scan'
os.makedirs(out_dir, exist_ok=True)
os.makedirs('data/nb5', exist_ok=True)

summary = []
T0 = time.time()

for factor in trap_factors:
    V_trap = V_trap_base * factor
    params = ModelParams(V_trap=V_trap)
    print(f"\n{'='*70}")
    print(f"Nb={Nb}, V_nn={V_nn}, V_nnn={V_nnn}, "
          f"trap factor={factor:.1f} (V_trap={V_trap:.4f}). "
          f"Total elapsed so far: {time.time()-T0:.0f}s")
    print('='*70)
    ...
    print(f"  GS: L={gs_L}, E={E100[0]:.6f}, gap={E100[1]-E100[0]:.4f}, "
          f"L_1st={L100[1]}")
    print("  Lowest 6: " + ", ".join(
        f"E-E0={E100[k]-E100[0]:+.4f}(L={L100[k]})" for k in range(6)))
    ...
    fstr = f'{factor:.1f}'.replace('.', 'p')
    tag = f'trap{fstr}x'
    out_npz = f'data/nb5/kagome_density_nb5_vnn5_vnnn5_{tag}.npz'
    np.savez(out_npz, V_nn=V_nn, V_nnn=V_nnn, V_trap=V_trap, trap_factor=factor,
    ...
    ax.set_title(rf'Nb={Nb}  $V_{{nn}}=V_{{nnn}}={V_nn:g}$, '
                 rf'trap{factor:g}$\times$ ($V_{{trap}}={V_trap:g}$)  '
                 f'(lowest {n_keep})\n'
    ...
    ax1.set_title(rf"trap{factor:g}$\times$ ($V_{{trap}}={V_trap:g}$)",
                  fontsize=12)
    ...
    summary.append({
        'factor': factor, 'V_trap': V_trap, 'L_GS': gs_L,
        'E_GS': float(E100[0]), 'gap': float(E100[1] - E100[0]),
        'L_1st': int(L100[1]), 'max_n': float(density.max()),
        'min_n': float(density.min())
    })

print(f"\n\n{'='*70}\nSummary:")
print(f"{'factor':>7}  {'V_trap':>8}  {'L_GS':>4}  {'E_GS':>10}  "
      f"{'gap':>7}  {'L_1st':>5}  {'max_n':>6}  {'min_n':>6}")
for s in summary:
    print(f"{s['factor']:7.1f}  {s['V_trap']:8.4f}  {s['L_GS']:4d}  "
          f"{s['E_GS']:10.4f}  {s['gap']:7.4f}  {s['L_1st']:5d}  "
          f"{s['max_n']:6.4f}  {s['min_n']:6.4f}")
